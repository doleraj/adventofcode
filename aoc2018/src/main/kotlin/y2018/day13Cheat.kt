...
class Day13(rawInput: List<String>) {
    ...
    fun solvePart2(): Pair<Int, Int> {
        collisions().toList() // Consume the entire sequence
        return carts.first { it.alive }.run { Pair(this.x, this.y) }
    }


    private fun collisions(): Sequence<Pair<Int, Int>> = sequence {
